...
s113h2o<-melt(s113h2o,id.vars = 'sample', measure.vars = c('d2_mu', 'd2_mu_2','d3_mu', 'd3_mu_2' ), value.name = "mu", na.rm = T)%>%
  mutate_at(vars(sample), funs (factor))
...
```{r off_plate_p }
s14_pvo<-get_p_values_off(max_gr$S6.14, dataC, "S6-14")
s45_pvo<-get_p_values_off(max_gr$S6.45, dataC, "S6-45")
s113_pvo<-get_p_values_off(max_gr$S6.113, dataC, "S6-113")
u5_pvo<-get_p_values_off(max_gr$U5.05,  dataC, "U5-05")
wlp_pvo<-get_p_values_off(max_gr$WLP.644, dataC, "WLP-644")
w34_pvo<-get_p_values_off(max_gr$W.34.70, dataC, "W-34/70")


samples_pvalues_off<-rbind(s14_pvo,s45_pvo,s113_pvo,u5_pvo,wlp_pvo, w34_pvo)


samples_p_sig_off<- samples_pvalues_off%>%
  mutate(sig = ifelse(test = (p_adj <= .05), 
                      yes = 1,
                      no = 0))
```
