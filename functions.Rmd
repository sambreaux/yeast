...
load meassure data
PLATE GROUPS
A01->A02, B02, C02, G02, A03, A04, A05, A06, C03, E03, G03, H03, B04, C04, D04, E04, F04, G04, C05, D05, E05, F05, G05, B06, C06, D06, E06, F06, G06, H06

B01->D01, F01, H01, D02, E02, F02, H02, B03, F03, H04, B05, G05

C01->E01, G01, D03

A07->A08, B08, C08, G08, A11, A09, A10, A12, C09, E09, G09, H09, B10, C10, D10, E10, F10, G10, C11, D11, E11, F11, G11, B12, C12, D12, E12, F12, G12, H12

B07->D07, F07, H07, D08, E08, F08, H08, B09, F09, H10, B11, G11
...
xdmso<-melt(xdmso,id.vars = 'sample', measure.vars = c('d2_mu', 'd2_mu_2','d3_mu', 'd3_mu_2','d4_mu' ,'d4_mu_2'), value.name = "mu", na.rm = T)%>%
...
xh2o<-melt(xh2o,id.vars = 'sample', measure.vars = c('d2_mu', 'd2_mu_2','d3_mu', 'd3_mu_2','d4_mu','d4_mu_2' ), value.name = "mu", na.rm = T)%>%
  mutate_at(vars(sample), funs (factor))
...
xETOH<-melt(xETOH,id.vars = 'sample', measure.vars = c('d2_mu', 'd2_mu_2','d3_mu', 'd3_mu_2','d4_mu','d4_mu_2' ), value.name = "mu", na.rm = T)%>%
  mutate_at(vars(sample), funs (factor))
...
#list_strain= list$strain where strain contains values to be tested (mu)
get_p_values_off<-function(list_strain, off_summary, current_sample){
 
  control_mu<-off_summary%>%
  select(mu, compound, strain)%>%
  rename_col_by_position(2, "sample")%>%
  rename_col_by_position(3, "variable") 
...
##
 off_dmso<- control_mu[control_mu$sample == "DMSO",]
  off_dmso<-off_dmso[off_dmso$variable == current_sample,]


xdmso<-s_solv$DMSO

xdmso<-melt(xdmso,id.vars = 'sample', measure.vars = c('d2_mu', 'd2_mu_2','d3_mu', 'd3_mu_2' ), value.name = "mu", na.rm = T)%>%
  mutate_at(vars(sample), funs (factor))%>%rbind(off_dmso)

xdmso$sample <- ordered(xdmso$sample,
                         levels = c("DMSO", "DMSO control", 
                                    "5-Fluorouracil" ,"6-Azauracil","CCCP",
                                    "Cinnamic acid","Cycloheximide","EDTA",
                                    "Fluconazole", "Guanidine hydrochloride","Miconazole Nitrate", 
                                    "Miltefosine","Nystatin ", "Triclosan"   ))
...
###
 ...
 off_h2o<- control_mu[control_mu$sample == "H2O",]
  off_h2o<-off_h2o[off_h2o$variable == current_sample,]
  
xh2o<-s_solv$H2O

xh2o<-melt(xh2o,id.vars = 'sample', measure.vars = c('d2_mu', 'd2_mu_2','d3_mu', 'd3_mu_2' ), value.name = "mu", na.rm = T)%>%
  mutate_at(vars(sample), funs (factor))%>%rbind(off_h2o)

xh2o$sample <- ordered(xh2o$sample,
                         levels = c( "H2O","H2O control ", 
                                    "2-Deoxy-D-glucose" , "3-Amino-1,2,4- triazole" , "4-Aminopyridine" ,"5-Fluorocytosine" , "Azaserine*",
                                    "Benzamidine" , "Blasticidin hydrochloride" ,"Caffeine", "Cetylpyridinium chloride",
                                    "Chlorpromazine hydrochloride ", "Compound 48/80*" , "Copper(II) sulfate", "Dequalinium Chloride" , 
                                    "Diamide", "DTT", "Hydroxyurea" , "Hygromycin B" , "methylmethane sulfonate", "Pentamidine Isethionate",
...
off_ETOH<- control_mu[control_mu$sample == "ETOH",]
  off_ETOH<-off_ETOH[off_ETOH$variable == current_sample,]
